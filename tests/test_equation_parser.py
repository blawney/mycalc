...
from src import parsers, custom_exceptions
...
class TestStringExpressionParser(unittest.TestCase):
    ...
    def test_backwards_arrow_raises_error(self):
        ...
        with self.assertRaises(custom_exceptions.MalformattedReactionDirectionSymbolException):
    ...
    def test_unidirectional_with_two_rate_constants_raises_exception(self):
        bad_eqn = 'A + B -> C,0.2,0.3'
        with self.assertRaises(custom_exceptions.ExtraRateConstantException):
            parsers.StringExpressionParser.parse(bad_eqn)
    ...
    def test_missing_reverse_rate_constant_for_bidirectional_reaction(self):
        bad_eqn = 'A + B <-> C,0.2'
        with self.assertRaises(custom_exceptions.MissingRateConstantException):
            parsers.StringExpressionParser.parse(bad_eqn)
        bad_eqn = 'A + B <-> C,0.2,'
        with self.assertRaises(custom_exceptions.MissingRateConstantException):
            parsers.StringExpressionParser.parse(bad_eqn)
    ...
    def test_non_numerical_rate_constant_for_unidirectional_reaction(self):
        bad_eqn = 'A + B -> C,a'
        with self.assertRaises(custom_exceptions.RateConstantFormatException):
            parsers.StringExpressionParser.parse(bad_eqn)
        bad_eqn = 'A + B <-> C,0.1,k'
        with self.assertRaises(custom_exceptions.RateConstantFormatException):
            parsers.StringExpressionParser.parse(bad_eqn)
    ...
    def test_missing_coefficient_in_reactants(self):
        ...
        with self.assertRaises(custom_exceptions.MalformattedReactionException):
            parsers.StringExpressionParser.parse(bad_eqn)

    def test_bad_symbols_raise_exceptions(self):
        with self.assertRaises(custom_exceptions.InvalidSymbolName):
    ...
    def test_adds_repeated_symbol_coefficients(self):
        ...
        reactants, products, fwd_k, rev_k, is_bidirectional = parsers.StringExpressionParser.parse(eqn)
        for r in reactants:
            if r.symbol == 'A':
